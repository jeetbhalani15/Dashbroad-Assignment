...
const NavbarIcons = styled.img`
  ...
  &:hover {
    transform: scale(1.1);
    filter: ${({ isDarkMode }) => (isDarkMode ? 'brightness(1.3)' : 'brightness(0.7)')};
  }
`;
...
const SearchBar = styled.input`
  ...
  &:focus {
    border: 1px solid ${({ isDarkMode }) => (isDarkMode ? '#FFFFFF66' : '#1470ec')};
    background-color: ${({ isDarkMode }) => (isDarkMode ? '#FFFFFF33' : '#e0f0ff')};
  }
`;
